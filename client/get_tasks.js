...
(async ()=>{
    ...
    if (gotTasks === undefined || gotTasks.length == 0){
        ...
        txt = `<div class="tasky"><h3>&nbsp;${aux}(Fri)</h3><p><button onclick='openNav()'>Add Task</button></p></div>`;
        const thisSec = document.createElement("section");
        thisSec.innerHTML = txt;
        const tab = document.createElement("table");
        txt = "<tr><th>Name</th><th>Stage</th><!--th>Implement</th><th>Status</th--><th>Work hours</th><th>Assigned</th><th>Actions</th></tr>";
        for (let idx=0;idx < gotTasks[jdx].length; idx++){
            console.log("status",gotTasks[jdx][idx]['stat']);
            if (gotTasks[jdx][idx]['stat'] == 100){
                stat_val="done";this_class="done_task";
                stat_width="100";
            }else if(gotTasks[jdx][idx]['stat'] == 0){
                stat_val="to-do";this_class="todo_task";
                stat_width="100";
            }else{
                stat_val="doing";this_class="doing_task";
                stat_width=gotTasks[jdx][idx]['stat'];
            }
            txt += `<tr><td><span>${gotTasks[jdx][idx]['name']}</span><div class='grey-fill float_left'><div class='${this_class}' style="width:${stat_width}%;height:12px;border-radius:5px;"></div></div><div class="col20 float_left">${gotTasks[jdx][idx]['stat']}%</div></td><td><span>${gotTasks[jdx][idx]['stage']}</span><br><span>${gotTasks[jdx][idx]['days']}</span></td><td class="centered">${gotTasks[jdx][idx]['work_hours']}</td><td><span>${gotTasks[jdx][idx]['assign']}</span><br><span>${gotTasks[jdx][idx]['received']}</span></td><td><button class="update" onclick="edit_book('${gotTasks[jdx][idx]['id']}',${jdx})">Update</button><button class="delete" onclick="del_book('${gotTasks[jdx][idx]['id']}')">Delete</button></td></tr>`;
        }//for idx
        tab.innerHTML = txt;
        thisSec.appendChild(tab);
        mainDiv.appendChild(thisSec);
        }//for jdx
    }//else
 })();
